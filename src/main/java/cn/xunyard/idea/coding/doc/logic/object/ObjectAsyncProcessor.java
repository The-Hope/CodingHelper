...
public class ObjectAsyncProcessor implements Runnable {
    ...
    public void run(String basePath) {
        ...
        try {
            ...
            List<JavaClass> objectJavaClassList = objectScanner.scan(basePath);
            if (AssertUtils.isEmpty(objectJavaClassList)) {
                log.error("过程终止，未发现有效源文件路径!");
                return;
            }

            ObjectResolver objectResolver = new ObjectResolver(objectJavaClassList, processContext);
            if (!objectResolver.run() && !configuration.isAllowInfoMissing()) {
                log.error("过程终止，修复注释缺失问题或者打开忽略开关以继续!");
                return;
            }

            ServiceDocumentRender documentRender = new ServiceDocumentRender(processContext, objectResolver.getServiceDescriberList());
            documentRender.run();
